...
export class TestAccessRepository {
  ...
  async seedUserSubscription(userId: string, subscriptionId: string, status: string, gracePeriodEnd?: string): Promise<void> {
  ...
  async deletePartition(tableName: string, pk: string): Promise<void> {
    const records = await this.ddb.send(new QueryCommand({
      TableName: tableName,
      KeyConditionExpression: 'PK = :pk',
