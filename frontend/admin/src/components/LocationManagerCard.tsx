...
const fieldClass = 'w-full rounded-md border border-slate-300 bg-white px-3.5 py-2.5 text-sm text-slate-900 shadow-sm placeholder:text-slate-400 transition focus:border-cyan-700 focus:outline-none focus:ring-2 focus:ring-cyan-700/20 disabled:bg-slate-100 disabled:text-slate-400 disabled:cursor-not-allowed';
const selectClass = 'w-full rounded-md border border-slate-300 bg-white px-3.5 py-2.5 text-sm text-slate-900 shadow-sm transition focus:border-cyan-700 focus:outline-none focus:ring-2 focus:ring-cyan-700/20 disabled:bg-slate-100 disabled:text-slate-400 disabled:cursor-not-allowed cursor-pointer';
      ...
      <div className="space-y-6">
        ...
        <div className="bg-slate-50/80 p-4 rounded-lg border border-slate-200 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className={fieldLabelClass} htmlFor="facility-address">Address</label>
              <input
                id="facility-address"
                type="text"
          ...
          <button
            ...
            className={`w-full flex items-center justify-center gap-1.5 rounded-md px-3.5 py-2 text-sm font-semibold text-white shadow-sm transition ${
              canCreateLocation ? 'bg-teal-700 hover:bg-teal-600 cursor-pointer' : 'bg-slate-300 text-slate-500 cursor-not-allowed opacity-60'
            }`}
        ...
        <div className="bg-cyan-50/40 p-4 rounded-lg border border-cyan-200 space-y-3">
          <div className="flex items-center justify-between border-b border-cyan-200 pb-2">
            <span className="text-xs font-bold uppercase tracking-wider text-cyan-900 flex items-center gap-2">
              <span className="w-5 h-5 rounded-full bg-cyan-700 text-white flex items-center justify-center text-xs">1</span>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className={fieldLabelClass} htmlFor="form1-location">Target Location</label>
            ...
            <div>
              <label className={fieldLabelClass} htmlFor="form1-scanner-name">Scanner Name</label>
              <input
          ...
          <button
            ...
            onClick={() => dispatch(createScannerThunk({ locationId: accessForm.locationId, name: accessForm.scannerName }))}
            disabled={!canAssignScanner}
            title={canAssignScanner ? 'Assign scanner hardware to location' : 'Select a location and enter scanner name'}
          >
