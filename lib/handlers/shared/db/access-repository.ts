...
export interface LockerUnlockParams {
  lockerId: string;
  scannerId: string;
  userId: string;
  entryId: string;
...
export interface AccessRepository {
  ...
  logLockerUnlock(params: LockerUnlockParams): Promise<void>;
}
...
export class DynamoDbAccessRepository implements AccessRepository {
            ...
            },
            {
              Put: {
                ...
                Item: {
                  ...
                  locker_id: params.scanner.assigned_locker_id,
  ...
  async logLockerUnlock(params: LockerUnlockParams): Promise<void> {
    ...
    try {
      await this.client.send(
        new PutCommand({
          TableName: this.entryLogsTableName,
          Item: {
            PK: `LOCKER#${params.lockerId}`,
            SK: `UNLOCK#${params.timestamp}#${logId}`,
            ...
            timestamp: params.timestamp,
            result: 'success',
            activity_type: 'locker_unlock_command',
            device_id: params.lockerId,
            scanner_id: params.scannerId,
            locker_id: params.lockerId,
            ttl: params.timestampEpochSeconds + 365 * 24 * 60 * 60,
          },
      ...
      console.warn('[DynamoDbAccessRepository] Failed to log locker unlock:', err);
    }
  }
}
